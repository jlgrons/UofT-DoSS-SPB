...
\begin{definition}
A function $f\colon \R \to \R$ is \emph{left continuous} at $x_0\in \R$ if for all $\epsilon >0$ there exists a $\delta>0$, such $\vert f(x_0)-f(x)\vert<\epsilon$ whenever $ x_0-\delta <x<x_0$. We call $f$ left continuous if it is left continuous at all $x\in \R$. Similarly, $f$ is \emph{right continuous} at $x_0\in \R$ if for all $\epsilon >0$ there exists a $\delta>0$, such $\vert f(x_0)-f(x)\vert<\epsilon$ whenever $x_0<x<x_0+\delta $.
\end{definition}
...
This proof essentially follows from the definitions (try it as an exercise if you do not see it).
...
\label{thm:mon_conv}
Suppose $(x_n)_{n\in \N}$ is an increasing sequence, i.e. $x_n\leq x_{n+1}$ for all $n\in \N$ and that it is bounded. Then the sequence converges. Furthermore, $\lim_{n\to \infty} x_n = \sup_{n\in \N} x_n$, where $\sup_{n\in \N} x_n := \sup\{x_n \; \colon \; n\in \N\}$.
\end{theorem}

The proof is omitted. A similar result can be obtained for decreasing sequences which are bounded below. In particular if $(x_n)_{n\in \N}$ is a bounded decreasing sequence, i.e. $x_n\geq x_{n+1}$ for all $n\in \N$, then the sequence converges and $\lim_{n\to \infty} x_n = \inf_{n\in \N} x_n$. Moreover, observe that if the sequence is increasing, but not bounded above, then $\lim_{n\to \infty} x_n = \infty$. We would like to use the monotone convergence theorem for arbitrary sequences. We do so by building an increasing sequence (or decreasing) sequence from an arbitrary one. First, we recall some facts about infima and suprema and introduce the convention that $\sup A = \infty$ if $A\subseteq \R$ is not bounded above and $\inf A= -\infty$ if $A$ is not bounded below.
...
If the sequence $(x_n)_{n\in \N}$ is not bounded above, then $\limsup_{n\to \infty} x_n = \infty$. Similarly, if the sequence $(x_n)_{n\in \N}$ is not bounded below, then $\liminf_{n\to \infty} x_n = -\infty$. This is inline with our convention that $\sup A = \infty$, if $A$ is not bounded above and $\inf A = -\infty$, if $A$ not bounded below. 
...
Provide examples to show that the other directions do not hold.
    ...
    \item Let $(x_n)$ be a sequence in $\R$. Show that $\lim_{n\to \infty} x_n = 0$ if and only if $\limsup_{n\to \infty} \vert x_n\vert = 0$.
\end{enumerate}
