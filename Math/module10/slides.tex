...
\begin{enumerate}
    ...
    \item Product rule: If $f$ and $g$ are differentiable at $x$, then so is $f+g$ with $$(fg)'(x) = f'(x) g(x) + f(x) g'(x).$$
...
Let $f\colon [a,b] \to \R$ be a function. We call a set of points $P=\{x_0, \ldots, x)n\}\subseteq [a,b]$ a \emph{partition} of $[a,b]$ if the following holds
\begin{equation*}
...
Given a partition $P=\{x_0, \ldots, x)n\}\subseteq [a,b]$ of $[a,b]$ and a set of points $T=\{t_1,\ldots, t_n\}\subseteq [a,b]$ such that $t_i \in [x_{i-1},x_i]$ for $i=1,\ldots, n$, we define the \emph{Riemann sum} $R(f,P,T)$ corresponding to $f,P,T$ as 
\begin{equation*}
