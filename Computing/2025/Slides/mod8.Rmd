...
\begin{align*}
M_Y(t) &= \mathbb{E}[e^{tY}] \\
&= \int e^{ty} \exp\left(\frac{y\theta - b(\theta)}{a(\phi)} + c(y, \phi)\right) dy \\
&= \exp\left(-\frac{b(\theta)}{\phi}\right) \int \exp\left(y\frac{\theta + \phi t}{a(\phi)} + c(y, \phi)\right) dy \\
&= \exp\left(-\frac{b(\theta)}{\phi}\right) \exp\left(\frac{b(\theta + \phi t)}{a(\phi)}\right) \int f(y \mid \theta + \phi t, \phi) dy \\
&= \exp\left(\frac{b(\theta + \phi t) - b(\theta)}{a(\phi)}\right)
\end{align*}
where the integral equals 1 because $f(y \mid \theta + \phi t, \phi)$ is a valid density/pmf.
...
Using the MGF:
\[
M'_Y(t) = \frac{d}{dt} \exp\left(\frac{b(\theta + \phi t) - b(\theta)}{a(\phi)}\right) = b'(\theta + \phi t) \exp\left(\frac{b(\theta + \phi t) - b(\theta)}{a(\phi)}\right)
\]
Evaluating at $t=0$:
\[
\mathbb{E}[Y] = M'_Y(0) = b'(\theta)
\]
...
\[
M''_Y(t) = \left[a(\phi) b''(\theta + \phi t) + (b'(\theta + \phi t))^2\right] \exp\left(\frac{b(\theta + \phi t) - b(\theta)}{a(\phi)}\right)
\]
Evaluating at $t=0$:
\[
\mathbb{E}[Y^2] = M''_Y(0) = a(\phi) b''(\theta) + (b'(\theta))^2
\]
Thus:
\[
\text{Var}(Y) = \mathbb{E}[Y^2] - (\mathbb{E}[Y])^2 = a(\phi) b''(\theta)
\]
