...
\end{equation}
and $h_k (p,q) \sim 2 n_{AA} \log p^{(k)} + n_{AO} \log(2p(1-p^{(k)}-q^{(k)})) + 2 n_{BB} \log q^{(k)} + n_{BO} \log (2p(1-p^{(k)}-q^{(k)})) + n_{AB} \log(2p^{(k)}q^{(k)}) + 2 n_O \log(1-p^{(k)}-q^{(k)})$
...
Then the \textbf{M-step} computes the MLE based on the likelihood in equation (3). To be specific, 
