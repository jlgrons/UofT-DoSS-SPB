...
A map $f: \mathbb{R}^n \to \mathbb{R}^m$ is called measurable if for any $B \in \mathcal{R}^m$ we have $f^{-1}(B) \in \mathcal{R}^n$. In other words, $f$ is measurable if the preimage of any Borel set in $\mathbb{R}^m$ is also a Borel set in $\mathbb{R}^n$.
\end{block}
...
Let $X = (X_1, \ldots, X_n)$ is a random vector on $(\Omega, \mathcal F, \mathbb P) \to \mathbb R^n$ and $f$ be a mesurable function on $\mathbb R^n$. Then $f(X)$ is a random variable on $(\Omega, \mathcal F, \mathbb P) \to \mathbb R$.
\end{block}
...
\begin{itemize}
    ...
    $$
    \mathbb{E}(X) = \int_{\mathbb{R}} x\;dF(x) = \int_{\mathbb{R}} x f_X(x)\;dx.
    $$
\end{itemize}
\end{block}}
\end{frame}
