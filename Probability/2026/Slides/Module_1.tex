...
\begin{frame}{Discrete and Continuous Probability}
...
The probability of an event is obtained by adding point probabilities.
\end{block}
\end{column}
...
The probability of an event is obtained by integrating the density.
\end{block}
\end{column}
\end{columns}
...
\begin{frame}{Discrete and Continuous Expectations}
...
\large
Again, a \textbf{sum} is used under discrete probability, while an \textbf{integral} is used under continuous probability.

\end{frame}

\begin{frame}{Why Can't We Always Use Sums?}
...
If we imitate discrete probability, then
\[
    1= \mathbb P(X \in \mathbb{R})
    =
    \sum_{x \in \mathbb{R}} \mathbb P(X=x)
    =
    \sum_{x \in \mathbb{R}} 0.
\]
...
The problem is not that each point has probability zero.

The problem is that we are trying to add \textbf{uncountably many} numbers.
\end{block}


\end{frame}

\begin{frame}{Countability Is the Key}

In ordinary calculus, infinite sums are defined for \textbf{countable} values:
\[
    \sum_{i=1}^{\infty} a_i.
\]
...
\[
    \sum_{x \in [0,1]} a_x
\]
is not a countable sum but an \textbf{uncountable} sum.

\vspace{3mm}

\begin{block}{Motivation}
Measure-theoretic probability is built around \textbf{countable} operations.

This motivates the definition of a \textbf{$\sigma$-algebra}.
\end{block}

\end{frame}

\begin{frame}{A Countable Approximation of the Real Line}

For each positive integer $n$, divide the real line into countably many intervals:
\[
...
\[
    \mathbb R
    =
    \bigcup_{i=-\infty}^{\infty}
    \left[\frac{i}{n}, \frac{i+1}{n}\right).
\]

These intervals are disjoint, so countable additivity gives
\[
...
\begin{block}{Key point}
Even though $\mathbb R$ is uncountable, it can be decomposed into \textbf{countably many} intervals.
\end{block}

\end{frame}

\begin{frame}{Approximating Expectation by Countable Sums}
...
\[
    ...
    \qquad i \in \mathbb Z,
\]
we approximate $X$ by the left endpoint of the interval containing it.
...
This formula uses only a \textbf{countable sum}, not an uncountable sum over individual points.
\end{block}

\end{frame}

\begin{frame}{From Approximation to Measure-Theoretic Probability}

As $n$ becomes larger, the intervals
\[
    \left[\frac{i}{n}, \frac{i+1}{n}\right)
\]
become smaller.

So the approximation should become more accurate
\[
    \mathbb E[X] \overset{?}{=} \lim_{n\to \infty}\sum_{i=-\infty}^{\infty}
    \frac{i}{n}
    \mathbb P\left(
        X \in
        \left[\frac{i}{n}, \frac{i+1}{n}\right)
...
These observations suggest that probabilities and expectations can be built from:

\begin{itemize}
    \item probabilities of sets,
    \item countable unions of sets,
    \item countable sums.
\end{itemize}
...
Measure-theoretic probability gives one unified language for both discrete and continuous probability by focusing on \textbf{sets} and \textbf{countable operations}.
\end{block}

\end{frame}

\begin{frame}{Introduction to Measure Theory}
...
\begin{frame}{Example: Tossing a Coin Twice}

Consider the sample space $\Omega=\{HH,HT,TH,TT\}$. \vspace{2mm}

One possible $\sigma$-algebra is the power set
\[
    2^\Omega,
\]
which contains all $2^4=16$ subsets of $\Omega$.
...
\begin{itemize}
    ...
    \item complements remain in $\mathcal F$,
    \item countable unions remain in $\mathcal F$.
\end{itemize}

\end{frame}

\begin{frame}{The Smallest and Largest $\sigma$-Algebras}

For any sample space $\Omega$, there are always two extreme examples.

\vspace{3mm}

\textbf{The trivial $\sigma$-algebra}
\[
\{
\emptyset,\Omega
\}.
...
Only two events are measurable.
...
Every subset of $\Omega$ is measurable.

\vspace{5mm}

\begin{block}{Observation}
Every $\sigma$-algebra lies between these two extremes:
\[
\{\emptyset,\Omega\}
\subseteq
\mathcal F
...
\begin{frame}{Generating a $\sigma$-Algebra}

Instead of specifying every measurable set, we often begin with a collection of subsets.
...
We then define $\sigma(\mathcal C)$ to be the \textbf{smallest $\sigma$-algebra containing every set in $\mathcal C$.}

\vspace{3mm}

Intuitively, $\sigma(\mathcal C)$

\begin{itemize}
\item include every set in $\mathcal C$,
\item include all complements,
\item include all countable unions,
\item repeat until no new sets are created.
\end{itemize}

\vspace{3mm}

This construction allows us to build useful $\sigma$-algebras from simple building blocks.

\end{frame}

\begin{frame}{Example: The Borel $\sigma$-Algebra}

On the real line, the most important $\sigma$-algebra is the
\textbf{Borel $\sigma$-algebra}.

It is generated by all open intervals:
...
for every open interval, and so are all sets obtained by repeatedly taking
...
\begin{frame}{Examples of Borel sets}
...
\begin{frame}{Problem Set}
    \textbf{Problem 1:} Prove that for a $\sigma$-algebra $\mc{F}$, if $A_1, A_2, \cdots \in \mc{F}$, then $\cap_{i = 1}^\infty A_i \in \mc{F}$.\\
    \vspace{0.1in}
    \textbf{Problem 2:} Prove monotonicity and subadditivity of measure $\mu$ on $\sigma$-algebra. \\
    \vspace{0.1in}
