---
title: "Module 3: Statistical inference (II)"
...
[Suppliment 3](https://jlgrons.github.io/UofT-DoSS-SPB/Computing/2026/Slides/supplement3.pdf) discusses the Wald test, the Score test and their equivalence to the LR test.
...
$$
\Delta_{n}= \ell\left(\widehat{\theta}_{n}\right)-\ell\left(\theta_{0}\right) =\log \left(\frac{L(\hat{\theta}_n\mid\mathbf{x})}{L\left(\theta_{0} \mid \mathbf{x}\right)}\right) =\log \left(\frac{\sup _{\theta \in \Theta}L(\theta \mid \mathbf{x})}{L\left(\theta_{0} \mid \mathbf{x}\right)}\right) \geq 0
$$
