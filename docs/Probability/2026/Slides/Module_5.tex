...
\begin{block}{Measurable function}
A map $f: \mathbb{R}^n \to \mathbb{R}^m$ is called measurable if for any $B \in \mathcal{R}^m$ we have $f^{-1}(B) \in \mathcal{R}^n$. In other words, $f$ is measurable if the preimage of any Borel set in $\mathbb{R}^n$ is also a Borel set in $\mathbb{R}^n$.
\end{block}

\textbf{Remark:} Recall that Borel $\sigma$-algebra contains almost all "nice" sets such as open sets, closed sets, and any sets that can be obtained by repeated use of countable union, intersections, and complements.

\end{frame}

\begin{frame}{Function of random vector revisited}

\begin{block}{Measurable function of random vector is a random variable}
Let $X = (X_1, \ldots, X_n)$ is a random vector on $(\Omega, \mathcal F, \mathbb P) \to \mathbb R^n$ and $f$ be a mesurable function on $\mathbb R^n$. Then $f(X)$ is a random variable on $(\Omega, \mathcal F, \mathbb P) \to \mathbb R^n$.
\end{block}

\textbf{Remark:} The definition of a measurable function is given so that the above holds.
...
Consider a random variable $X$ , the expectation of $X$ is defined by $\mathbb{E}(X)$, where 
\begin{itemize}
    ...
    $$
    \mathbb{E}(X) = \int_{\mathbb{R}^n} x\;dF(x) = \int_{\mathbb{R}^n} x f_X(x)\;dx.
    $$
\end{itemize}
\end{block}}
\end{frame}
...
\textbf{Intuition}: How do the random variables behave on average?\\
\uncover<2->{
\begin{block}{Measure theoretic definition of Expectation (informal)}
Consider a random variable $X$ , the expectation of $X$ is defined by $\mathbb{E}(X)$, where 

\[
\mathbb{E}(X) = \lim_{n\to \infty} \sum_{i=-\infty}^\infty \frac{i}{n}\mathbb{P}\left(X \in \left(\frac{i}{n}, \frac{i+1}{n}\right]\right).
\]
\end{block}}

\textbf{Remark:} Recall that Borel $\sigma$-algebra and random variables are defined so that $\mathbb{P}\left(X \in \left(\frac{i}{n}, \frac{i+1}{n}\right]\right)$ is well defined.
\end{frame}


\begin{frame}{Expectation}

\begin{block}{Expectation of function of random vector}
...
\begin{itemize}
    ...
    \item Continuous random vector in $\mathbb{R}^n$
    $$
    \mathbb{E}(g(X)) = \int_{\mathbb{R}^n} g(x)\;dF(x) = \int_{\mathbb{R}^n} g(x)f_X(x)\;dx.
    $$
\end{itemize}
\end{block}
\end{frame}
